...
REQUESTS = BoundedSemaphore(32)
ORIGIN_REQUESTS = 4
_origin_requests: dict[str, BoundedSemaphore] = {}
_origin_lock = Lock()


def _origin_limiter(origin: str) -> BoundedSemaphore:
    with _origin_lock:
        return _origin_requests.setdefault(origin, BoundedSemaphore(ORIGIN_REQUESTS))
...
def fetch(url: str, timeout: float, limit: int = 10_000_000) -> bytes:
    ...
    with _origin_limiter(origin), REQUESTS:
        with _origin_lock:
            if _retry_at.get(origin, 0) > time.time():
                raise OSError(f"Retry-After cooldown active for {origin}")
        try:
